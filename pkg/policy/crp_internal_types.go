...
// CRPPeerRule is the agent's internal representation of a peer rule component
// derived from a CiliumResolvedPolicy. It uses a map for efficient identity lookups
// for matching peer identities.
...
// a single CiliumResolvedPolicy Custom Resource. It uses CRPPeerRule
// internally and converts SubjectIdentities to a map for efficient lookup.
type CRPRuleSet struct {
	...
	// to which this set of rules applies. This is a map for efficient lookup, converted from
	// the slice in v2alpha1.CiliumResolvedPolicySpec.SubjectIdentities.
	SubjectIdentities map[identity.NumericIdentity]struct{}
}

// IdentityRuleChain is a map of CRPRules keyed by the source policy UID.
type IdentityRuleChain map[string]*CRPRuleSet
...
type SubjectPolicyState struct {
	...
	MatchingCRPUIDset IdentityRuleChain
...
// Helper function to create a CRPRuleSet from a CiliumResolvedPolicySpec CR.
// This is where the conversion of SubjectIdentities slice to map would occur,
// and v2alpha1.CRPPeerRule to CRPPeerRule.
func ConvertToCRPRuleSet(spec *v2alpha1.CiliumResolvedPolicySpec) (*CRPRuleSet, error) {
	// Basic validation (e.g., spec != nil)
	if spec == nil {
		return nil, fmt.Errorf("CiliumResolvedPolicySpec is nil")
	}

	// Convert slice of subject identities to map for efficient lookup
	subjectIdentitiesMap := make(map[identity.NumericIdentity]struct{})
	for _, id := range spec.SubjectIdentities {
		subjectIdentitiesMap[id] = struct{}{}
	}

	// Construct SourcePolicyUID (example, needs refinement based on actual PolicyRef fields)
	// For KNP, PolicyRef.UID might be most reliable. For CNP/CCNP, Name+Namespace (if applicable).
	sourceUID := spec.PolicyRef.UID
	if sourceUID == "" {
		return nil, fmt.Errorf("unable to construct SourcePolicyUID from PolicyRef")
	}

	// Helper function to convert v2alpha1 peer rules to internal representation
	convertPeerRules := func(rules []v2alpha1.CRPPeerRule) []CRPPeerRule {
		if rules == nil {
			return nil
		}
		agentRules := make([]CRPPeerRule, len(rules))
		for i, rule := range rules {
			...
			for _, id := range rule.MatchingPeerIdentities {
				matchingIdentitiesMap[id] = struct{}{}
			}
			agentRules[i] = CRPPeerRule{
	...
	return &CRPRuleSet{
		...
		IngressRules:      convertPeerRules(spec.IngressRuleIdentities),
		IngressDenyRules:  convertPeerRules(spec.IngressDenyRuleIdentities),
		EgressRules:       convertPeerRules(spec.EgressRuleIdentities),
		EgressDenyRules:   convertPeerRules(spec.EgressDenyRuleIdentities),
		SubjectIdentities: subjectIdentitiesMap,
	}, nil
}
...
type ResolvedIdentityPolicyUpdate struct {
	...
	// CrpSpec is the resolved identity policy to be applied.
...
func (rules IdentityRuleChain) resolveL4IngressPolicy(policyCtx PolicyContext) (ingressPolicyMap L4PolicyMap, ingress bool, err error) {
	...
	for _, crpRuleSet := range rules {
		if len(crpRuleSet.IngressRules) > 0 || len(crpRuleSet.IngressDenyRules) > 0 {
			ingress = true
		}
		// check if the CRPRuleSet is nil or has no ingress rules to resolve
		if crpRuleSet != nil && len(crpRuleSet.IngressRules) > 0 || len(crpRuleSet.IngressDenyRules) > 0 {
			_, err := crpRuleSet.resolveIngressPolicy(policyCtx, &state, result)
			if err != nil {
				return nil, ingress, fmt.Errorf("failed to resolve ingress policy for CRPRuleSet %s: %w", crpRuleSet.SourcePolicyUID, err)
			}
			state.ruleID++
		}
	}
...
func (rules IdentityRuleChain) resolveL4EgressPolicy(policyCtx PolicyContext) (egressPolicyMap L4PolicyMap, egress bool, err error) {
	result := NewL4PolicyMap()
	...
	for _, crpRuleSet := range rules {
		if len(crpRuleSet.EgressRules) > 0 || len(crpRuleSet.EgressDenyRules) > 0 {
			egress = true
		}
		// check if the CRPRuleSet is nil or has no egress rules to resolve
		if crpRuleSet != nil && len(crpRuleSet.EgressRules) > 0 || len(crpRuleSet.EgressDenyRules) > 0 {
			_, err := crpRuleSet.resolveEgressPolicy(policyCtx, &state, result)
			if err != nil {
				return nil, egress, fmt.Errorf("failed to resolve egress policy for CRPRuleSet %s: %w", crpRuleSet.SourcePolicyUID, err)
			}
			state.ruleID++
		}
	}
...
func (rule *CRPRuleSet) resolveIngressPolicy(
	policyCtx PolicyContext,
	state *traceState,
	result L4PolicyMap,
) (L4PolicyMap, error) {
	found, foundDeny := 0, 0
	for _, peerRule := range rule.IngressRules {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to merge ingress policy for CRPRuleSet %s: %w", rule.SourcePolicyUID, err)
		}
		if cnt > 0 {
			found += cnt
		}
	...
	for _, peerRule := range rule.IngressDenyRules {
		if len(peerRule.MatchingPeerIdentities) == 0 {
			continue // Skip rules with no matching identities
		}
		// make a copy of the map instead of passing the original
		...
		if err != nil {
			return nil, fmt.Errorf("failed to merge ingress deny policy for CRPRuleSet %s: %w", rule.SourcePolicyUID, err)
		}
		if cnt > 0 {
			foundDeny += cnt
		}
...
func (rule *CRPRuleSet) resolveEgressPolicy(
	policyCtx PolicyContext,
	state *traceState,
	result L4PolicyMap,
) (L4PolicyMap, error) {
	found, foundDeny := 0, 0
	for _, peerRule := range rule.EgressRules {
		if len(peerRule.MatchingPeerIdentities) == 0 {
			continue // Skip rules with no matching identities
		}
		// make a copy of the map instead of passing the original
		...
		if err != nil {
			return nil, fmt.Errorf("failed to merge Egress policy for CRPRuleSet %s: %w", rule.SourcePolicyUID, err)
		}
		if cnt > 0 {
			found += cnt
		}
	...
	oldDeny := policyCtx.SetDeny(true)
	defer policyCtx.SetDeny(oldDeny)
	for _, peerRule := range rule.EgressDenyRules {
		if len(peerRule.MatchingPeerIdentities) == 0 {
			continue // Skip rules with no matching identities
		}
		// make a copy of the map instead of passing the original
		...
		if err != nil {
			return nil, fmt.Errorf("failed to merge Egress deny policy for CRPRuleSet %s: %w", rule.SourcePolicyUID, err)
		}
		if cnt > 0 {
			foundDeny += cnt
		}
...
func mergeIngressWithIdentities(policyCtx PolicyContext, fromEndpoints map[identity.NumericIdentity]struct{}, auth *api.Authentication, toPorts, icmp api.PortsIterator, ruleLabels stringLabels, resMap L4PolicyMap) (int, error) {
	...
	if fromEndpoints == nil || len(fromEndpoints) == 0 {
...
func mergeEgressWithIdentities(policyCtx PolicyContext, toEndpoints map[identity.NumericIdentity]struct{}, auth *api.Authentication, toPorts, icmp api.PortsIterator, ruleLabels stringLabels, resMap L4PolicyMap) (int, error) {
	...
	// short-circuit if no endpoint is selected
	if toEndpoints == nil || len(toEndpoints) == 0 {
		// return error here, always expect fromEndpoints to be populated
		// by centralized policy controller. For wildcard endpoint selector,
		// worldwide reserved identity is used.
		return found, fmt.Errorf("no fromEndpoints provided for ingress policy merge")
